...
Calculates the Sortino Ratio, a downside risk-adjusted performance measure.
Contrary to the Sharpe Ratio, only deviations below the minimum acceptable returns are included in the calculation of the risk (downside deviation instead of standard deviation).
...
@inline sortino_ratio(returns; multiplier=1.0, MAR=0.0) =
    mean_excess(returns, MAR) / downside_deviation(returns, MAR; method=:full) * sqrt(multiplier)
